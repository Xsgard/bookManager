...
(function($){
	var WIN = $(window),
		...
		DELETE = function (id) {
			delete $.dialog.list2[id];
			if ($.isEmptyObject($.dialog.list2))
			{
				ZINDEX = zIndex;
			}
		};
	...
	$.dialog.get = function (id) {
		return $.dialog.list2[id];
	}
	//关闭所有对话框，参数o-布尔值，是否静默关闭
	$.dialog.close = function (o) {
		for (var i in $.dialog.list2) {
			$.dialog.list2[i].close(o);
		};
	};
	$.dialog.fn = $.dialog.prototype = {
		...
		_init : function(o) {
			...
			$.each($.dialog.list2, function (x, y) {
		...
		_event : function () {
			...
			e.wrapper.unbind(EVENT.A).bind(EVENT.A, $.proxy(function () {
				...
				{
					this.zIndex = ZINDEX;
					$.dialog.list2[this.id].wrapper.css('z-index', ZINDEX);
					ZINDEX++;
					this._setTop();
				}
			}, e));
